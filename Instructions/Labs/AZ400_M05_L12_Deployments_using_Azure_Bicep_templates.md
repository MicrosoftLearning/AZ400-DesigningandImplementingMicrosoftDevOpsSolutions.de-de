...
1. Ändern Sie den Standardwerts des `publicIPAllocationMethod`-Parameters in Zeile 20 von `Dynamic` in `Static`.

1. Ändern Sie den Standardwerts des `publicIpSku`-Parameters in Zeile 27 von `Basic` in `Standard`.
...
#### Aufgabe 3: Ändern der Vorlage „simple-windows-vm“ zur Verwendung des Vorlagenmoduls

In dieser Aufgabe ändern Sie die Vorlage `simple-windows-vm.bicep` so, dass sie auf das Vorlagenmodul verweist, das Sie in der vorherigen Aufgabe erstellt haben.
...
1. Warten Sie, bis die Bereitstellung abgeschlossen ist, und überprüfen Sie die Ergebnisse.
   ...
   > **Hinweis:** Denken Sie daran, der Pipeline die Berechtigung zur Verwendung der zuvor erstellten Dienstverbindung zu erteilen.
